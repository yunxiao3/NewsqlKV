...
std::unordered_map<std::string_view, int> uid_index;  // after sort, build uid_index // user_id char[128]
...
void build_index() {
  ...
  for (int i = 0; i < size; i++) {
    ...
    uid_index[std::string_view(users[i].user_id, 128)] = i;
...
int solveUserId(const void *column_key, size_t column_key_len, void *res) {
  ...
  auto find = uid_index.find(std::string_view(uid, 128));
